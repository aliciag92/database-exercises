...
GROUP BY last_name; #Eldridge, Erbe, Erde, Erie, Etalle
...
SELECT gender, COUNT(first_name) AS number_of_employees, first_name 
FROM employees
...
FROM employees
GROUP BY username
ORDER BY COUNT(*) DESC; #Yes there are duplicates.

#-Bonus: How many duplicates are there?
-- step-by-step
...
SELECT COUNT(t.number_of_duplicates) AS unique_duplicate_usernames, SUM(t.number_of_duplicates) AS number_of_duplicate_usernames
FROM (SELECT LOWER
				(CONCAT
					(SUBSTR(first_name, 1, 1),
					SUBSTR(last_name, 1, 4),
...
/* 
unique_duplicate_usernames			number_of_duplicate_usernames
13251								27403
*/
